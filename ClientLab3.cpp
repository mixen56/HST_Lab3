...
int main(int argc, char* argv[]) {
    ...
    //cout << "[Client " << RANK_MPI << "] Recv matrix size = " << matrix_size << endl;

    // recv vector size
    MPI_Recv(&vector_size, 1, MPI_INT, MASTER, SIZE_VECTOR_TAG,
            MPI_COMM_WORLD, &ignored_status);
    //cout << "[Client " << RANK_MPI << "] Recv part size = " << vector_size << endl;
    ...
    //
    //cout << "[Client " << RANK_MPI << "] Recv matrices from " << MASTER << endl;
    for (int i = 0; i < vector_size; i ++) {
    ...
    //cout << "[Client " << RANK_MPI << "] Start calcs" << endl;
    vector<vector<double>> result_vector = get_max(matrix_vector_part, calcs_time);
    //cout << "[Client " << RANK_MPI << "] Stop calcs" << endl;
    ...
    if (RANK_MPI != MASTER) {
        ...
        // send vector
        //cout << "[Client " << RANK_MPI << "] Send part result vector to " << MASTER << endl;
